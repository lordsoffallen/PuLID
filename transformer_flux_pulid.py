...
class FluxTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        # pulid input
        if ('id_embeddings' in joint_attention_kwargs and
            joint_attention_kwargs['id_embeddings'] is not None):
            id_embeddings = joint_attention_kwargs['id_embeddings']
            id_masks = []
            for id_mask in joint_attention_kwargs['id_masks']:
                id_masks.append(id_mask[None, :, None].repeat(
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            if self.training and self.gradient_checkpointing:
                ...
                if id_embeddings is not None and index_block % self.pulid_double_interval == 0:
                    for id_weight, id_embedding, id_mask in zip(
                        id_weights, id_embeddings, id_masks
                    ):
                        hidden_states = (hidden_states + id_weight *
                                         self.pulid_ca[ca_idx](id_embedding, hidden_states) * id_mask)
                    ca_idx += 1
...
class FluxTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin,
    ...
    def forward(
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            if self.training and self.gradient_checkpointing:
                ...
                encoder_hidden_states, hidden_states, encoder_hidden_states_base = block(
                    ...
                    additional_kwargs=additional_kwargs if index_block %
                                                           joint_attention_kwargs[
                                                               'double_inject_blocks_interval'] == 0 else {
                        k: v for k, v in additional_kwargs.items() if
                ...
                # pulid compute
                if id_embeddings is not None and index_block % self.pulid_double_interval == 0:
                    for id_weight, id_embedding, id_mask in zip(id_weights,
                                                                id_embeddings,
                                                                id_masks):
                        hidden_states = hidden_states + id_weight * self.pulid_ca[
                            ca_idx](id_embedding, hidden_states) * id_mask
                    ca_idx += 1
        ...
        if encoder_hidden_states_base is not None:
            hidden_states_base = torch.cat([encoder_hidden_states_base, hidden_states],
                                           dim=1)
        else:
        ...
        for index_block, block in enumerate(self.single_transformer_blocks):
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states, hidden_states_base = block(
                    ...
                    additional_kwargs=additional_kwargs if index_block %
                                                           joint_attention_kwargs[
                                                               'single_inject_blocks_interval'] == 0 else {
                        k: v for k, v in additional_kwargs.items() if
                        k != 'regional_attention_mask'},
                    # delete attention mask to avoid region control
                )
